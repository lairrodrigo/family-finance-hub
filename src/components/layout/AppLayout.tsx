            ...
            {tabs.map(({ to, icon: Icon, label }) => {
              ...
              return (
                <NavLink
                  ...
                  to={to}
                  className={cn(
                    "flex flex-col items-center justify-center gap-1.5 px-4 py-1 rounded-2xl transition-all duration-300",
                    isActive ? "text-white bg-white/[0.03]" : "text-muted-foreground hover:text-white/70"
                  )}
                >
                  <Icon className={cn("h-5 w-5 transition-transform duration-300", isActive && "scale-110")} />
                  <span className={cn("text-[9px] font-bold uppercase tracking-[0.15em] transition-opacity", isActive ? "opacity-100" : "opacity-40")}>
                    {label}
                  </span>
                </NavLink>
              );
